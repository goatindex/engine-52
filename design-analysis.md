...
*Update (exploration v0.2):* "spend" is better modelled not as one fate but as a **ladder of destinations** with increasing recoverability — permanent, tapped, team pile, system pile, global feeder — routed by a card/action **default sink** so it costs no per-turn decision (pattern candidate SPEND HAS A DESTINATION, 27). And the hand itself is better treated as a quantity that **breathes** — growing, shrinking, or turning over at role-specific rates — rather than a fixed allotment (THE BREATHING HAND, 26); shrinkage becomes one designed trajectory, not the default.
...
9. **Decking-out has three readings, and the choice is thematic.** An emptied deck is currently an implicit single clock (game over). It has three coherent readings: **game-end** (the consumable arc), **function-disabled** (the resource the deck represents switches off until repaired/reset), and **function-degraded** (the system keeps running at reduced capacity — a soft slide). If cards recirculate (the destination ladder, §5 update), depletion is hard to reach and stops being central; if they don't, depletion *is* the clock. Make this explicit with a visible clock layer rather than letting the endgame arrive by accident (pattern candidate CLOCKS MAKE TIME VISIBLE, 29) — and prefer one primary clock per game so tracking doesn't compete with the card economy.
